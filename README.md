...
DATABASE_SKIP_MIGRATION |boolean                    |false                                    |**Only used by the docker image.** If set to "true", skips the `gomp db migration up` on container startup. Can be useful when overriding the default container command. Also accepts `GOMP_DATABASE_SKIP_MIGRATION`.
DATABASE_URL            |string                     |file:data/data.db?_pragma=foreign_keys(1)|The url (path, connection string, etc) to use with the associated database driver when opening the database connection. When using the provided docker image, the `DATABASE_URL_FILE` (or `GOMP_DATABASE_URL_FILE`) variable can be set to the path to a file containing the value.
...
SECURE_KEY              |[]string                   |ChangeMe                                 |Used for session authentication. Recommended to be 32 or 64 ASCII characters. When using the provided docker image, the `SECURE_KEY_FILE` (or `GOMP_SECURE_KEY_FILE`) variable can be set to the path to a file containing the value.
