...
type sqliteTagDriver struct {
	*sqliteDriver
}

func (d sqliteTagDriver) Create(recipeID int64, tag string) error {
	return d.tx(func(tx *sqlx.Tx) error {
		return d.CreateTx(recipeID, tag, tx)
	})
}

func (d sqliteTagDriver) CreateTx(recipeID int64, tag string, tx *sqlx.Tx) error {
	_, err := tx.Exec(
...
func (d sqliteTagDriver) DeleteAll(recipeID int64) error {
	return d.tx(func(tx *sqlx.Tx) error {
		return d.DeleteAllTx(recipeID, tx)
	})
}

func (d sqliteTagDriver) DeleteAllTx(recipeID int64, tx *sqlx.Tx) error {
	_, err := tx.Exec(
...
func (d sqliteTagDriver) List(recipeID int64) (*[]string, error) {
	var tags []string
...
func (d sqliteTagDriver) Find(filter *models.TagsFilter) (*[]string, error) {
