...
func NewHandler(renderer *render.Render, cfg *conf.Config, upl upload.Driver, model *models.Model) http.Handler {
	...
	h.apiMux.GET("/api/v1/users/:userID", h.requireAuthentication(h.requireSelf(h.getUser)))
	h.apiMux.PUT("/api/v1/users/:userID/password", h.requireAuthentication(h.requireSelf(h.putUserPassword)))
	h.apiMux.GET("/api/v1/users/:userID/settings", h.requireAuthentication(h.requireSelf(h.getUserSettings)))
	h.apiMux.PUT("/api/v1/users/:userID/settings", h.requireAuthentication(h.requireSelf(h.putUserSettings)))
